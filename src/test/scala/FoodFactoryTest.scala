...
class FoodFactoryTest extends AnyFunSuite{
  ...
  val factory: Set[Food] = Food(100, 10)(strategy)
  ...
  test("The coords should all be between boundaries"){
    assert(factory.count(f => f.center.x >= 0 && f.center.x <= 100 && f.center.y >= 0 && f.center.y <= 100) == 100)
  }
