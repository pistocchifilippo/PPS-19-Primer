...
case class Visualizer(environment: Environment) extends JPanel {
  ...
  override def paint(g: Graphics): Unit = {
    setSize(SIMULATOR_WIDTH/2, SIMULATOR_HEIGHT/2)
    val g2 = g.asInstanceOf[Graphics2D]
    g2.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON)
    g2.setRenderingHint(RenderingHints.KEY_RENDERING, RenderingHints.VALUE_RENDER_QUALITY)
    g2.clearRect(0, 0, SIMULATOR_WIDTH, SIMULATOR_HEIGHT)
    println(environment.creatures.size)
    println(environment.food.size)
  ...
  def display(/*environment: Environment*/): Unit = repaint()
}
